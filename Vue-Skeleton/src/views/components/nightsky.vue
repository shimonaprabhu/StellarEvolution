...
import testData from "../../assets/data/constellations.json"; /* Example of reading in data direct from file*/

export default {
  name: "NightSky",

  props: {
    myNightskyData: Array,
  },
  mounted() {
    ...
    // console.log("Data Passed down as a Prop  ", this.myNightskyData)
  },
  methods: {
    drawNightSky(data, id) {
      ...
      function getStart() {
        ...
        console.log("ORG", raStart, decStart);
      }
      function move() {
        console.log("move");
        var raFinish = projection.invert(d3.pointer(event))[0];
        var decFinish = fixedProjection.invert(d3.pointer(event))[1];

        var raRotate = raFinish - raStart;
        var decRotate = decFinish - decStart;
        // var raRotate = raFinish;
        // var decRotate = decFinish;

        var rotate = projection.rotate();
        // var newCenter = [rotate[0] + raRotate, rotate[1] + decRotate];
        var newCenter = [raStart, decStart];
        console.log("RA", raStart, "DEC", decStart, "NEW CENTER", newCenter);
        draw(geoConstellations, newCenter);

        decStart = fixedProjection.invert(d3.pointer(event))[1];
      }
      var drag = d3.drag().on("start", getStart).on("drag", move);
      canvas.call(drag);
      ...
      function draw(constellations, center) {
        var min = 0,
          minDistance = distance(
            projection(constellations[0].properties.center)
          );
